...
class AI(object):
    ...
    def go(self, chessboard):
        ...
        self.candidate_list = self.greedy_approach(chessboard, self.candidate_list)
        return self.candidate_list

    # Decide the possible positions for placing a chess piece
    def possible_positions(self, chessboard) -> list:
        positions = []
    ...
    def check_legal(self, chessboard, x, y) -> bool:
        flag = False
    ...
    # use greedy to decide where to put the chess piece
    def greedy_approach(self, chessboard, possible_positions) -> list:
        greedy_decision = []
        min_points = 1000
        for position in possible_positions:
            points = self.greedy_count_points(chessboard, position[0], position[1])
            if points < min_points:
                min_points = points
                greedy_decision.append(position)
            else:
                greedy_decision.insert(0, position)
        return greedy_decision

    # greedy: calculate the number of chess pieces been reversed
    def greedy_count_points(self, chessboard, x, y) -> int:
        points = 0
        for direction in self.directions:
            count = 0
            possible_x = x + direction[0]
